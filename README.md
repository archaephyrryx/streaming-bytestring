...
chars :: MonadResource m => ByteStream m ()
...
data ByteStream m r =
    ...
    | Go (m (ByteStream m r))
```
which shows that a ByteStream is either:
- An empty stream's terminal value, or,
- a `Chunk` containing an initial strict `ByteString` segment followed by
  the rest the stream, or,
- a monadic effect that returns the rest of the stream.

The definition is recursive, both chunks and monadic actions carry a
continuation of the stream, which allows further nesting.  A sequence of
chunks alternating with monadic actions that return the next chunk is the
shape of many *source* ByteStreams, that read data from a file or network
socket.

Though, as mentioned above, the terminal value is often ultimately just `()`,
it is crucially more interesting in various intermediate transformations of a
basic `ByteStream`.  But first a few preliminaries:

### Functor Instance:

The type `ByteStream m` is a functor in the final parameter `r`.  The functor
is defined rather simply, it leaves alone both the stream and the intermediate
monadic actions, and just transmutes the terminal value, with the
transformation delayed until the end of the stream is reached:
```haskell
...
Thus
```
    fmap :: (r -> s) -> ByteStream m r -> ByteStream m s
```
has no effect on the actual stream of data, its only effect is to apply the
function to the terminal value.  But the thunks representing the rest of the
stream can become somewhat more complex.

### Monad Instance:

The type `ByteStream m` is also a monad, here things get a bit more
interesting.  The slightly simplified definition is:
```haskell
...
```
Here a *pure* value `r` is just an empty `ByteStream` with `r` as its terminal
value.  But sequencing two `ByteStream`s one after the other with `(>>)`
combines the two streams by discarding the terminal value of the first and
replacing the `Empty` tail with the second stream, which will ultimately
provide the terminal value of the combined stream.  Thus `(>>)` is stream
concatenation, but it is worth noting that the two terminal values may be of
different types:
```haskell
    (>>) :: ByteStream m r -> ByteStream m s -> ByteStream m s
```
The bind operator `(>>=)` is rather similar, but here the new tail of the
stream is a function of its original terminal value.  The effect is lazy, nothing
new happens until the the stream end is reached, at which point finally the
...
```
But it may be worth keeping in mind that multiple transformations of the stream
can result in more complex thunks for its lazy continuation.  Keeping the
transformation chains short may improve performance.

### Interaction with `Stream`

A `Stream` is an even more general structure that is parameterised over an
...
stream itself just captures the sequencing of content and effects.  Therefore,
the functor `f` is typically also parameterised by the payload type.  The
most commonly seen functor seen in the `streaming` library is `Of` which
is essentially just a pair, that is strict only in its first element:
```haskell
    data Of a b = !a :> b
```
This typically holds data of type `a`, and stream tails of some type
`b = Stream f m r`.  In this way, a `Stream (Of a) m r` represents a monadic
sequence of elements of type `a` followed by the rest of the stream, which is
periodically punctuated by monadic effects, and ultimately terminates at an
element of type `r`.  This is largely the same as our `ByteStream` above, but more
general because the data content is modeled via an arbitrary functor, not just
a chunk holding a strict `ByteString`.

We're finally ready to discuss the real topic of this section, which is what
does it mean when the functor `f` in a `Stream f m r` is `ByteStream m`???
This was mentioned briefly in the line splitting example above, but now we take
the time to look at this construction more closely:
```
-- Ad hoc, but hopefully suggestive name, not part of the API, but useful in
-- this README.
type ByteStreamStream = Stream (ByteStream m) m r
```
...
Here a `Step` encapsulates a monadic ByteStream, which can consist of an
arbibtrary (even unbounded) number of chunks interspersed with monadic effects
to read more of the ByteStream, until the inner `ByteStream` ends, exposing its
terminal value.  But now that value is not an uninteresting `()`, it is
instead the rest of the outer `Stream`!
...
constructors of `ByteStream`, but this not typically necesary or desirable.

### lineSplit walk through
...
`Q.stdin` to a stream of sub-streams (our ad hoc `ByteStreamStream` defined
above), where each stream except perhaps the last consists of 10 logical lines.
...
Next the resulting `ByteStreamStream` is folded back to a simple `ByteStream`
...
`ByteStreamStream` type back to a regular `ByteStream`, namely `Q.concat` (not
...
etc.) as a `ByteStreamStream` (i.e. `Stream (ByteStream m) m r`), it is often
...
In the context of subdivided streams (our ByteStreamStream type) the functions
...
value, which in a ByteStreamStream holds the next substream.  This makes the
...
```
- The first `splitAt` turns a `ByteStream` into an initial stream of at most
  the requested length with the rest of the stream as a terminal value.
- The second, `drained` performs all the effects of the terminal value of
  such a stream, discarding the content, and ultimately returns the terminal
